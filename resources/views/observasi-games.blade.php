...
<div class="dashboard-card mb-5">
    <div class="card-body p-4 p-md-5">
        ...
        <div class="d-flex align-items-center mb-4">
            <div>
                <h5 class="fw-bold mb-1" style="color: #0f172a;">Observasi Games</h5>
                <p class="text-muted mb-0" style="font-size: 0.875rem;">Kelola dan catat hasil observasi peserta pada setiap sesi games.</p>
            </div>
        </div>
        
        <!-- Nav Materi (Underline UI) -->
        <ul class="nav modern-tabs" id="materi-tabs" role="tablist">
            @foreach($games as $index => $gamesItem)  
            <li class="nav-item">
                <a class="nav-link @if($index == 0) active @endif" 
                   id="materi-tab-{{ $gamesItem->id }}" 
                   data-bs-toggle="tab" 
                   href="#materi-{{ $gamesItem->id }}" 
                   role="tab">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="12" rx="2" ry="2"></rect><path d="M12 12h.01"></path><path d="M17 12h.01"></path><path d="M7 12h.01"></path></svg>
                    {{ $gamesItem->nama_games }}
                </a>
            </li>
            @endforeach
        </ul>
        ...
        <div class="tab-content" id="materi-tab-content">
            ...
            <div class="tab-pane fade @if($index == 0) show active @endif" 
                ...
                <div class="section-header">
                    ...
                    <div>
                        <h5 class="mb-1 fw-bold" style="color: #0f172a;">{{ $gamesItem->nama_games }}</h5>
                        <div class="text-muted" style="font-size: 0.875rem;">Lembar Penilaian Keaktifan Peserta</div>
                    </div>
                </div>

                <form method="POST" action="{{ route('observasigames.store') }}">
                    @csrf
                    <input type="hidden" name="games_id" value="{{ $gamesItem->id }}">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <input type="checkbox"
                                                ...
                                                value="1"
                                                @if(isset($absensi[$gamesItem->id . '_' . $p->id]) && $absensi[$gamesItem->id . '_' . $p->id]->hadir)
                                                    checked
                                                @endif
                                            >
                                        </td>
                                        ...
                                        @foreach($kategori as $kat)
                                            ...
                                            <td>
                                                <input type="checkbox"
                                                    class="form-check-input custom-cb checkbox-afektif materi-{{ $gamesItem->id }} peserta-{{ $p->id }}"
                                                    name="nilai[{{ $gamesItem->id }}][{{ $p->id }}][afektif][]"
                                                    value="1"
                                                    @if(isset($observasi[$gamesItem->id . '_' . $p->id]) && $i < ($observasi[$gamesItem->id . '_' . $p->id]->afektif ?? 0) / 10)
                                                        checked
                                                    @endif
                                                >
                                            </td>
                                            ...
                                            <td>
                                                <input type="checkbox"
                                                    class="form-check-input custom-cb checkbox-kognitif materi-{{ $gamesItem->id }} peserta-{{ $p->id }}"
                                                    name="nilai[{{ $gamesItem->id }}][{{ $p->id }}][kognitif][]"
                                                    value="1"
                                                    @if(isset($observasi[$gamesItem->id . '_' . $p->id]) && $i < ($observasi[$gamesItem->id . '_' . $p->id]->kognitif ?? 0) / 10)
                                                        checked
                                                    @endif
                                                >
                                            </td>
                                            ...
                                            <td>
                                                <input type="checkbox"
                                                    class="form-check-input custom-cb checkbox-psikomotorik materi-{{ $gamesItem->id }} peserta-{{ $p->id }}"
                                                    name="nilai[{{ $gamesItem->id }}][{{ $p->id }}][psikomotorik][]"
                                                    value="1"
                                                    @if(isset($observasi[$gamesItem->id . '_' . $p->id]) && $i < ($observasi[$gamesItem->id . '_' . $p->id]->psikomotorik ?? 0) / 10)
                                                        checked
                                                    @endif
                                                >
                                            </td>
                                        ...
                                        <td class="bg-light fw-bold" style="color: #0f172a;">
                                            <span id="avg-{{ $gamesItem->id }}-{{ $p->id }}">0</span>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    peserta.forEach(p => {
        games.forEach(m => {
            ...
            // Tambahkan event listener ke semua checkbox peserta per materi
            aspekList.forEach(aspek => {
...
document.addEventListener('DOMContentLoaded', function () {
    @if(session('success'))
        // Menampilkan SweetAlert2 saat berhasil disimpan
        Swal.fire({
            icon: 'success',
            ...
            position: 'top-end',  // Menempatkan popup di kanan atas
            toast: true,  // Membuatnya menjadi toast, sehingga auto-hide
            didClose: () => {
                // Bisa disesuaikan, jika perlu tindakan setelah close
            }
        });
    @endif
});
