...
window.darkMode = function (el) {
    ...
    function applyDarkMode() {
        ...
        cardNavSpan.forEach((span) => span.classList.add("text-white"));
        hr.forEach((hrEl) => hrEl.classList.replace("dark", "light"));
        hr_card.forEach((hrCardEl) =>
            hrCardEl.classList.replace("dark", "light")
        );
        text_btn.forEach((btn) =>
            btn.classList.replace("text-dark", "text-white")
        );
        text_span.forEach((span) =>
            span.classList.replace("text-dark", "text-white")
        );
        text_strong.forEach((strong) =>
            strong.classList.replace("text-dark", "text-white")
        );
        text_nav_link.forEach((link) =>
            link.classList.replace("text-dark", "text-white")
        );
        secondary.forEach((sec) => {
            sec.classList.replace("text-secondary", "text-white");
            sec.classList.add("opacity-8");
        });
        bg_gray_100.forEach((bg) =>
            bg.classList.replace("bg-gray-100", "bg-gray-600")
        );
        btn_text_dark.forEach((btn) =>
            btn.classList.replace("text-dark", "text-white")
        );
        card_border.forEach((card) => card.classList.add("border-dark"));
    ...
    function applyLightMode() {
        ...
        navbarBrand.src = navbarBrandImg.includes("logo-ct.png")
            ? navbarBrandImg.replace("logo-ct", "logo-ct-dark")
            : navbarBrand.src;
        ...
        cardNavSpan.forEach((span) => span.classList.remove("text-white"));
        hr.forEach((hrEl) => hrEl.classList.replace("light", "dark"));
        hr_card.forEach((hrCardEl) =>
            hrCardEl.classList.replace("light", "dark")
        );
        text_btn.forEach((btn) =>
            btn.classList.replace("text-white", "text-dark")
        );
        text_span.forEach((span) =>
            span.classList.replace("text-white", "text-dark")
        );
        text_strong.forEach((strong) =>
            strong.classList.replace("text-white", "text-dark")
        );
        secondary.forEach((sec) => {
            sec.classList.replace("text-white", "text-dark");
            sec.classList.remove("opacity-8");
        });
        bg_gray_600.forEach((bg) =>
            bg.classList.replace("bg-gray-600", "bg-gray-100")
        );
        ...
        svg.forEach((svgElement) => {
            if (svgElement.hasAttribute("fill")) {
                svgElement.setAttribute("fill", "#252f40");
            }
        });
    }
