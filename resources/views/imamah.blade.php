...
<div class="dashboard-card mb-5">
    <div class="card-body p-4 p-md-5">
        ...
        <div class="d-flex align-items-center mb-4">
            <div>
                <h5 class="fw-bold mb-1" style="color: #0f172a;">Observasi Pendalaman (Kajian)</h5>
                <p class="text-muted mb-0" style="font-size: 0.875rem;">Kelola dan catat hasil observasi peserta pada setiap sesi kajian.</p>
            </div>
        </div>
        
        <!-- Nav Materi (Underline UI) -->
        <ul class="nav modern-tabs" id="materi-tabs" role="tablist">
            @foreach($imamah as $index => $imamahItem) 
            <li class="nav-item">
                <a class="nav-link @if($index == 0) active @endif" 
                   id="materi-tab-{{ $imamahItem->id }}" 
                   data-bs-toggle="tab" 
                   href="#materi-{{ $imamahItem->id }}" 
                   role="tab">
                    ...
                    {{ $imamahItem->nama_kajian }}
                </a>
            </li>
            @endforeach 
        </ul>

        <!-- Tab Content per imamah -->
        <div class="tab-content" id="materi-tab-content">
            @foreach($imamah as $index => $imamahItem)
            <div class="tab-pane fade @if($index == 0) show active @endif" 
                 id="materi-{{ $imamahItem->id }}" 
                 role="tabpanel">
                ...
                <div class="section-header">
                    ...
                    <div>
                        <h5 class="mb-1 fw-bold" style="color: #0f172a;">{{ $imamahItem->nama_kajian }}</h5>
                        <div class="text-muted" style="font-size: 0.875rem;">Lembar Penilaian Keaktifan Peserta</div>
                    </div>
                </div>

                <form method="POST" action="{{ route('observasiImamahKajian.store') }}">
                    @csrf
                    <input type="hidden" name="imamah_id" value="{{ $imamahItem->id }}">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="checkbox"
                                                ...
                                                value="1"
                                                @if(isset($absensi[$imamahItem->id . '_' . $p->id]) && $absensi[$imamahItem->id . '_' . $p->id]->hadir)
                                                    checked
                                                @endif
                                            >
                                        </td>
                                        ...
                                        @foreach($kategori as $kat)
                                            ...
                                            <td>
                                                <input type="checkbox"
                                                    ...
                                                    name="nilai[{{ $imamahItem->id }}][{{ $p->id }}][afektif][]"
                                                    value="1"
                                                    @if(isset($observasi[$imamahItem->id . '_' . $p->id]) && $i < ($observasi[$imamahItem->id . '_' . $p->id]->afektif ?? 0) / 10)
                                                        checked
                                                    @endif
                                                >
                                            </td>
                                            ...
                                            <td>
                                                <input type="checkbox"
                                                    ...
                                                    name="nilai[{{ $imamahItem->id }}][{{ $p->id }}][kognitif][]"
                                                    value="1"
                                                    @if(isset($observasi[$imamahItem->id . '_' . $p->id]) && $i < ($observasi[$imamahItem->id . '_' . $p->id]->kognitif ?? 0) / 10)
                                                        checked
                                                    @endif
                                            >
                                            </td>
                                            ...
                                            <td>
                                                <input type="checkbox"
                                                    ...
                                                    name="nilai[{{ $imamahItem->id }}][{{ $p->id }}][psikomotorik][]"
                                                    value="1"
                                                    @if(isset($observasi[$imamahItem->id . '_' . $p->id]) && $i < ($observasi[$imamahItem->id . '_' . $p->id]->psikomotorik ?? 0) / 10)
                                                        checked
                                                    @endif
                                                >
                                            </td>
                                        ...
                                        <td class="bg-light fw-bold" style="color: #0f172a;">
                                            <span id="avg-{{ $imamahItem->id }}-{{ $p->id }}">0</span>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
